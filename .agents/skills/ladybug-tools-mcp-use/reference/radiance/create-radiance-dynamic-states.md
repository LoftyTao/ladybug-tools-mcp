...
- `RAD_create_state_geometry`: creates `StateGeometry`.
- `RAD_create_shade_state`: use for Honeybee Shade targets.
- `RAD_create_subface_state`: use for Honeybee Aperture or Door targets.
- `RAD_setup_dynamic_group`: sets group identifier and states on selected model objects.

## Code Mode Pattern

```python
modifier = await call_tool("RAD_create_opaque_modifier", {
    "garden_root": garden_root,
    "identifier": "winter_screen_modifier",
    "rgb_reflectance": 0.32,
    "return_object_dict": False
})
state_geo = await call_tool("RAD_create_state_geometry", {
    "garden_root": garden_root,
    "identifier": "winter_screen_geo",
    "geometry": {"type": "Face3D", "boundary": [[0, 0, 2.1], [1, 0, 2.1], [1, 1, 2.1], [0, 1, 2.1]]},
    "modifier": modifier["target"]
})
state = await call_tool("RAD_create_shade_state", {
    "garden_root": garden_root,
    "modifier": modifier["target"],
    "shades": [state_geo["object_dict"]]
})
setup = await call_tool("RAD_setup_dynamic_group", {
    "garden_root": garden_root,
...
- `RAD_create_state_geometry` requires exactly one of `geometry` or `vertices`.
