...
- `RAD_create_wea_from_weather_file`: from Garden `weather_file` target.
- `RAD_create_ashrae_clear_sky_wea`: theoretical clear sky from a Ladybug `Location`.
- `RAD_create_cie_standard_sky`: single-timestep Radiance `gensky` CIE sky.
- `RAD_create_climate_based_sky`: single-timestep Radiance `gendaylit` sky from irradiance or illuminance.
- `RAD_create_sky_matrix`: WEA/weather/location to SkyMatrix.

## Code Mode Patterns

```python
wea = await call_tool("RAD_create_wea_from_weather_file", {
...
```python
cie = await call_tool("RAD_create_cie_standard_sky", {
    "garden_root": garden_root,
...
- `RAD_create_sky_matrix` returns a `sky_matrix` target.
- `summary_view.patch_count` is `145` for default Tregenza or `577` for high-density Reinhart.

## Stop Conditions

- `RAD_create_sky_matrix` accepts exactly one source shape: `wea_target`, `weather_target`/`epw_path`, or `location`.
