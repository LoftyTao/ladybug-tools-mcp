...
- Treat OpenDSS as a local URBANopt CLI 1.2.0 bundle path requiring an offline runtime pack. If `config_get_runtime_config.summary_view.engines.urbanopt.opendss_python_deps.initialized` or `offline_runtime_pack.ready` is not `true`, report the dependency-initialization blocker and stop; do not download dependencies or call `uo install_python` from the Agent run.
