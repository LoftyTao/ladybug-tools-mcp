...
- For a local no-external-TCP check, run `scripts/verify_urbanopt_no_network.py` around the bundled `uo run` command and keep the generated report under the approved Codex Agent output location. Treat this as `no_external_tcp_capture`; the verifier also checks stdout, retained `eplusout.err` files, and standard EnergyPlus outputs so a failed EnergyPlus/URBANopt run is `urbanopt_simulation_failed` or `urbanopt_energy_outputs_unreasonable` even if no external TCP was observed. Inspect `energy_output_diagnostics` before promoting the evidence: it should show `finished.job`, SQL/HTML outputs, no `failed.job`, and clean ERR files. Inspect `bundle_log_audit`: it must show project `.bundle/config` using the expected local CLI gem bundle, include `runner_conf` when the project has one, show bundled `bundle exec uo` command evidence, list exact bundle-path hits in `bundle_path_evidence_sources`, and keep `online_fetch_markers` empty across stdout/stderr, `mcp_urbanopt_*.json`, and `run/**/*.log` / `run/**/*.osw` simulation evidence files. Treat those simulation evidence files as required local-bundle evidence: missing files mean `simulation_log_evidence_missing`, and files that exist but never echo the expected local `.bundle` path mean `simulation_log_bundle_echo_missing`; both are partial evidence and cannot be called `local-bundle runtime pass`.
- To collect optional `fully offline pass` evidence, rerun the same retained project on a physically disconnected host or use `scripts/verify_urbanopt_no_network.py --apply-firewall-deny --require-fully-offline-pass` from an elevated shell. Without administrator permission the verifier writes `firewall_setup_failed`; without real disconnected/firewall-deny isolation the optional hard gate leaves sampled `no_external_tcp_capture` evidence partial/non-zero instead of promoting it.
