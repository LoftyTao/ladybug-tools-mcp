...
- URBANopt CLI subprocesses are sanitized for no-network validation: proxy variables and REopt/NREL API key variables are stripped before launch, and `LADYBUG_MCP_NO_NETWORK=true` is set.
