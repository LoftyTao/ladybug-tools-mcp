...
- Locate the Room or Face with `HB_search_model_objects`.
...
2. Pass `matches[i].target` to `LB_honeybee_room_to_visualization_set` or `LB_honeybee_face_to_visualization_set`.
...
vis = await call_tool("LB_honeybee_room_to_visualization_set", {
    "garden_root": garden_root,
