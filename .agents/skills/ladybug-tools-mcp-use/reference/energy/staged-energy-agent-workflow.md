...
Tools: `HB_search_model_objects`, `HB_create_apertures_by_parameters`, `HB_create_shades_by_parameters`.
...
Tools include `EP_search_energy_library_objects`, `EP_create_window_construction`, `EP_create_construction_set`, `EP_create_schedule_ruleset`, `EP_create_setpoint`, `EP_create_program_type`, `EP_search_hvac_templates`, `EP_create_ideal_air_system`, `EP_setup_simple_ventilation_properties`, `EP_setup_airflow_network`, `EP_create_zone_ventilation_fan`, `EP_create_pv_properties`, `EP_create_electric_load_center`, `HB_edit_room`, `HB_edit_shade`, `HB_edit_model`, and `HB_validate_model`.
...
Resources/tools: `weather://catalog`, `EP_import_local_weather`, `EP_search_weather_files`, `EP_start_simulation`, `EP_poll_simulation`.

Read `weather://catalog`, select a station, then call `EP_import_local_weather` with `garden_root` and `source_path="weather://files/<station>"`. Pass the returned `weather_file` target into `EP_start_simulation.weather_target`. Stop after the run is `running`, `completed`, or `failed`.

If the catalog lacks the location, direct the user to `https://climate.onebuilding.org/` and stop the weather stage. Resume after the extracted local EPW/DDY/STAT folder is available; do not replay earlier stages.

## Stage E: Outputs

Goal: read bounded results from an existing completed or failed run.

Tools: `EP_poll_simulation`, `EP_list_run_outputs`, `EP_read_eui`, `EP_read_errors`, and SQL/DataCollection result readers when requested.
