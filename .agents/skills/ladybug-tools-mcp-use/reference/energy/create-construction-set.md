...
1. Optionally search standards with `EP_search_energy_library_objects`.
...
5. Pass the ConstructionSet target to `HB_edit_room.construction_set`.

## Baseline Pattern

```python
construction_set = await call_tool("EP_create_construction_set", {
...
```python
window = await call_tool("EP_create_window_construction", {
