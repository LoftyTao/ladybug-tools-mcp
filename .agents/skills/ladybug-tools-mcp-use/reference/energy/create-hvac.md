...
- Search Room targets explicitly and pass `matches[i].target` to `HB_edit_room`.
...
1. Create or find an HVAC target with `EP_search_hvac_templates` or `EP_create_ideal_air_system`.
2. Create a Setpoint with `EP_create_setpoint`.
3. Search Rooms with `HB_search_model_objects(object_type="room")`.
4. Attach HVAC and Setpoint with `HB_edit_room`.
5. Validate the model.

## Template HVAC Pattern

```python
hvac = await call_tool("EP_search_hvac_templates", {
...
- `HB_edit_room.summary_view.updated_fields` includes `hvac` and `setpoint`.
- Room search shows the edited Energy properties.
- `HB_validate_model.summary_view.is_valid == true`.

## Stop Conditions

- Do not invent `create_template_hvac`; `EP_search_hvac_templates` combines search and instantiation.
