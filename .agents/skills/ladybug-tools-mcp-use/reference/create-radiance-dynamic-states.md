...
- For Garden checks, use `get_garden`, `get_base_model`, or `search_honeybee_model_objects`. Do not call an inner `search` tool inside `execute`; Code Mode domain tools are reached only through `await call_tool(...)`.
