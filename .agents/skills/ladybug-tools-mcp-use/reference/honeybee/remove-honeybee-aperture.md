...
- Locate the Aperture with `HB_search_model_objects`.
...
```python
apertures = await call_tool("HB_search_model_objects", {
    "garden_root": garden_root,
    "object_type": "aperture",
    "identifier": "Front_Aperture"
})

removed = await call_tool("HB_remove_aperture", {
    "garden_root": garden_root,
...
- `HB_remove_aperture.summary_view.removed_count >= 1`.
