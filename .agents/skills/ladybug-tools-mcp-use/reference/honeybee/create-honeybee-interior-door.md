...
- If rooms are adjacent but not related, run `HB_relate_model` before creating the interior door.
...
3. Pass one Surface wall target to `HB_create_door`.
...
door = await call_tool("HB_create_door", {
    "garden_root": garden_root,
...
For an ordinary exterior door, search an `Outdoors` wall face and call `HB_create_door` with width/height/sill values. The paired-door rule only applies to `Surface` walls.
