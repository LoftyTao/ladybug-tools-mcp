...
- A Garden exists. If not, run `GD_create` first.
...
2. Call `HB_create_model` with the exact `garden_root`.
...
5. Call `GD_get_base_honeybee_model` to confirm the Garden registry points to the model you just created.
...
model = await call_tool("HB_create_model", {
...
`HB_create_model` can receive `add_objects` with full Honeybee object dictionaries such as `Room`, `Face`, `Aperture`, `Door`, or `Shade`. This is a deterministic object-dict path. Do not pass typed targets from create tools into `add_objects`; create tools already persist their own results.
...
- `GD_get_base_honeybee_model` returns a `honeybee_model` target for the same model.

## Stop Conditions

- Do not use list or search tools as base-model confirmation. Confirm with `GD_get_base_honeybee_model`.
