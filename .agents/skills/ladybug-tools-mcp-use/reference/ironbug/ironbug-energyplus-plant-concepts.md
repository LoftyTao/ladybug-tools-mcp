...
Local tool-test directions include air-cooled EIR chiller + FCU, repaired
fluid-cooler condenser-water loop + FCU, and PlantLoop EIR heat pump with
cooling/heating `HeatPump:PlantLoop:EIR` writer coverage. Treat these as local
...
validation; those routes remain blocked or partial. Compact branch-contained
`IB_NodeProbe` plus SystemNodeReset is a local candidate path, not an accepted
Agent playbook.
