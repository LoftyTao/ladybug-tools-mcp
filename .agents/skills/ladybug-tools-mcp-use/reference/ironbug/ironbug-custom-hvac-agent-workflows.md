...
- Rooms must already have enough Energy setup for simulation, such as
  ...
  missing room setpoints with `EP_create_setpoint` plus `HB_edit_room`
...
Tool names in this reference use the complete `IB_*` names returned by Code Mode
`search` / `get_schema`; preserve the exact case when calling them.
...
5. Create source-backed `IB_*` components.
6. Apply with `IB_apply_to_honeybee_model`.
7. Run standard Energy with `EP_start_simulation` and poll
   `EP_poll_simulation` until EnergyPlus finishes.
8. For a completed run, call `EP_list_run_outputs`, `EP_read_errors`, and
   `EP_read_eui` with the same `run_target`; take `.err` and `.sql` from the
   returned output `path` values, then use `EP_read_result_data` to inventory
   available SQL outputs before reading exact `output_names` for end-use checks.
9. Accept the new capability only after EnergyPlus completes, ERR/SQL/EUI are
   readable, and the ventilation intensity, end-use components, and EUI are
   reasonable for the served area, occupancy, and intended system. A suspicious
   result is a blocker: calibrate the inputs and rerun before reporting a pass.
   Do not rebuild the HVAC graph or start another run after an accepted result.
...
- Do not use `IB_load_profile_plant`, generic PlantLoop tools,
  public relationship helpers, explicit OperationScheme tools, or
  `IB_add_hvac_component_fallback`.
...
- `IB_outdoor_air_system.oa_stream_targets` should usually be
...
- For `IB_apply_to_honeybee_model`, pass only one room
  selection mode: `room_identifiers`, `room_targets`, or
  `apply_to_all_rooms=true`.
- For `EP_read_eui`, use the current schema from the tool if unsure; do not
  retry with guessed aliases after EUI already exists.
- If a write partially succeeds, resume from persisted targets and repair the
  existing objects; do not recreate the model or stack duplicate graphs.
- Before editing or deleting an existing component, call
  `IB_search_model_objects` with `object_type="component"` and inspect the
  match's `summary_view.reference_owners`,
  `summary_view.active_graph_memberships`, and `summary_view.is_orphan`.
- Use `IB_update_model_object` with the typed component `target`, one declared
  `field_name`, and exactly one update mode; carry its
  `updated_model_target` forward because embedded EMS, equipment, zone, and
  loop copies are refreshed automatically.
- Use `IB_remove_component` only for a typed component target with no owners or
  active graph memberships; stop when it is referenced or active.
  Call it with `cleanup_orphans=true` and no `target` only for an explicit
  orphan-cleanup request, then re-search to confirm removal.
- Keep new writes on the reviewed `create_ironbug_*` tools; do not use raw
  dictionaries, all-model replacement, or the retired source generator.
- ZoneHVAC ERV supply and exhaust fans must be created with
  `IB_fan_on_off` or `IB_fan_system_model` only; do not pass
  `IB_fan_constant_volume` to `IB_zone_equipment_energy_recovery_ventilator`.
...
1. `IB_thermal_zone`.
2. `IB_fan_on_off` with `"Autosize"` maximum flow.
3. `IB_coil_heating_electric`.
4. `IB_coil_cooling_dx_single_speed`.
5. `IB_zone_equipment_ptac` with fan,
...
1. `IB_thermal_zone`.
2. `IB_fan_on_off`.
3. `IB_coil_heating_dx_single_speed`.
4. `IB_coil_cooling_dx_single_speed`.
5. `IB_coil_heating_electric` for supplemental heat.
6. `IB_zone_equipment_pthp`.
...
1. `IB_thermal_zone`.
2. `IB_fan_on_off`.
3. `IB_coil_heating_water` with numeric water flow, UA, and rated
   temperatures.
4. `IB_zone_equipment_unit_heater`.
5. Hot-water loop: `IB_pump_constant_speed`,
   `IB_district_heating_water`, `IB_plant_loop_hot_water`
...
1. `IB_thermal_zone`.
2. `IB_fan_on_off`.
3. `IB_coil_cooling_water`.
4. `IB_coil_heating_water`.
5. `IB_zone_equipment_unit_ventilator_cooling_heating` with fixed small
   outdoor air flow.
6. Chilled-water loop: pump + `IB_district_cooling`, cooling coil
   as demand.
7. Hot-water loop: pump + `IB_district_heating_water`, heating coil
   as demand.

Do not use `IB_district_heating`.
...
`IB_zone_equipment_unit_ventilator_cooling_only` and
`IB_zone_equipment_unit_ventilator_heating_only`; do not route them
...
1. `IB_thermal_zone`.
2. `IB_fan_on_off`.
3. `IB_coil_heating_water`; use numeric UA and maximum water flow
   when using the UA/design-water-flow input method.
4. `IB_coil_cooling_water`.
5. `IB_zone_equipment_four_pipe_fan_coil`.
...
1. Chilled-water loop with pump + cooling source and FCU cooling coils as
   ...
   Use `IB_district_cooling` unless the prompt asks for a
   chiller/tower.
2. Hot-water loop with pump + `IB_district_heating_water` and all
...
1. `IB_thermal_zone` with
   `is_air_terminal_before_zone_equipments=True`.
2. `IB_coil_heating_water` with numeric sizing/rated values.
3. `IB_air_terminal_single_duct_constant_volume_reheat`.
4. `IB_fan_constant_volume`.
5. `IB_air_loop_branches` with `[[zone]]`.
6. `IB_air_loop_hvac` with fan on supply and branches on demand.
7. Hot-water loop with pump + `IB_boiler_hot_water`, reheat coil as
   demand.
...
1. `IB_chiller_electric_eir` with `condenser_type="WaterCooled"`.
2. Condenser loop with condenser pump + `IB_cooling_tower_variable_speed`
...
4. Hot-water loop with pump + `IB_district_heating_water` and FCU
...
1. `IB_heat_exchanger_fluid_to_fluid` with
...
6. Create `IB_air_loop_branches` with one branch per ThermalZone.
7. Create `IB_air_loop_hvac` with `[outdoor_air_system, supply_fan]`
...
1. DOAS air loop: mechanical ventilation, outdoor-air controller with
   ...
   `IB_air_loop_branches`, and AirLoopHVAC.
...
Do not use `IB_add_hvac_component_fallback`, `IB_list_hvac_component_types`, or
...
4. OnOff fan.
5. `IB_zone_equipment_terminal_unit_variable_refrigerant_flow`.

Then call `IB_air_conditioner_variable_refrigerant_flow` with all
...
1. Matching ThermalZone.
2. `IB_air_terminal_vav_no_reheat` with autosized max
...
`IB_air_terminal_vav_reheat`. Hot-water loop demand is the central
...
`IB_pump_variable_speed` for the hot-water loop. The chilled-water
...
1. `IB_coil_cooling_dx_multi_speed` with explicit stage identifiers,
   rated capacities, sensible heat ratios, COPs, and air-flow rates.
2. `IB_coil_heating_dx_multi_speed` with explicit stage identifiers,
   rated capacities, COPs, and air-flow rates.
3. `IB_fan_on_off`.
4. `IB_coil_heating_electric` for supplemental heat.
5. `IB_air_loop_unitary_heat_pump_air_to_air_multi_speed` with the
   cooling coil, heating coil, FanOnOff, supplemental coil, controlling
   ThermalZone, and matching speed-level flow fields.
6. `IB_setpoint_manager_scheduled` on the air-loop supply path.
7. A constant-volume no-reheat terminal, `IB_air_loop_branches`, and
   `IB_air_loop_hvac`.
...
For room-level EnergyRecoveryVentilator equipment, create the sensible/latent
heat exchanger, one allowed fan for each supply/exhaust side, and optionally an
ERV controller, then call
`IB_zone_equipment_energy_recovery_ventilator` with
`heating_exchanger_target`, `supply_fan_target`, `exhaust_fan_target`,
`controller_target`, and `thermal_zone_target` using the returned typed targets.
The tool authors zone equipment; it is not the OutdoorAirSystem heat-recovery
path. Use defensible `supply_air_flow_rate` and `exhaust_air_flow_rate` values in
`m3/s`, or `Autosize` when no design basis is available.
...
2. Pass it to `IB_outdoor_air_system` through
...
1. No-reheat DOAS air terminal.
2. `IB_coil_heating_water_baseboard_radiant` with exact
   `"Autosize"` strings for water flow and design capacity.
3. `IB_zone_equipment_baseboard_radiant_convective_water` with
...
1. `IB_coil_heating_low_temp_radiant_const_flow` per served room.
2. `IB_zone_equipment_low_temp_radiant_const_flow` with the heating
...
`IB_zone_equipment_low_temp_radiant_var_flow` topology now has
