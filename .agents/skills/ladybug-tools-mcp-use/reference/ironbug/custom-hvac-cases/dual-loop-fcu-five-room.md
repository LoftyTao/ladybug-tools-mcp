# Case Skill: dual_loop_fcu_five_room

## Applicable Scenario

Use this case when the request matches the retained prompt: `给五个房间添加风机盘管，冷热水都由独立环路供应。`. Keep the system family and served-room list exactly aligned with this case (["Room1", "Room2", "Room3", "Room4", "Room5"]) unless you intentionally switch to the family workflow for a variant.

## User Prompt And Keywords

- Prompt: `给五个房间添加风机盘管，冷热水都由独立环路供应。`
- Keywords: Room1-Room5, FCU, separate chilled-water loop, separate hot-water loop
...
Use the five-room FCU-only path.
...
case_id = "dual_loop_fcu_five_room"
...
```jsonc
{
  ...
  "eui": 123.456,
  "err_path": "runs/energy/dual_loop_fcu_five_room_run/annual_energy_use/run/eplusout.err",
  "sql_path": "runs/energy/dual_loop_fcu_five_room_run/annual_energy_use/run/eplusout.sql",
  "blocker": null
}
```
...
Do not add DOAS, AirLoopHVAC, generic PlantLoop, or load-profile plant demand.
