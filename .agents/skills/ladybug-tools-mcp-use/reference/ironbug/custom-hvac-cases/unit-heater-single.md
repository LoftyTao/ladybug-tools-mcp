...
4. Create `detailed_hvac_zone_equipment_unit_heater` with fan, heating coil, and
   ThermalZone.
5. Create hot-water loop: constant-speed pump + `detailed_hvac_district_heating_water`
   on supply, heating coil on demand.
...
```jsonc
{
  ...
  "eui": 123.456,
  "err_path": "runs/energy/unit_heater_single_run/annual_energy_use/run/eplusout.err",
  "sql_path": "runs/energy/unit_heater_single_run/annual_energy_use/run/eplusout.sql",
  "blocker": null
}
```
