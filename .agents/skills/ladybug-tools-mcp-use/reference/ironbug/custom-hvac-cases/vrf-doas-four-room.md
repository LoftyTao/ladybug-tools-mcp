...
- Prompt: `对 Room1 到 Room4 添加 VRF，并加新风系统。`
- Keywords: Room1-Room4, VRF, DOAS, fresh air
...
2. Create matching ThermalZone with that terminal.
3. Create VRF cooling coil, VRF heating coil, OnOff fan, and VRF terminal.

Then:

1. Create one VRF root with all four VRF terminal targets.
2. Create DOAS air loop: mechanical ventilation controller, outdoor-air
   controller, supply fan, outdoor-air system with `oa_stream_targets=[supply_fan]`,
   branches `[[zone1], [zone2], [zone3], [zone4]]`, and AirLoopHVAC.
3. Apply, run Energy, read EUI/ERR/SQL.
...
Do not create plant loops, NoAirLoop, load-profile plant demand, or generic
PlantLoop tools.
