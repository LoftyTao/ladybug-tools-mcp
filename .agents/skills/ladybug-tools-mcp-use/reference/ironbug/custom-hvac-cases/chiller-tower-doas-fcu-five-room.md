# Case Skill: chiller_tower_doas_fcu_five_room

## Applicable Scenario

Use this case when the request matches the retained prompt: `给五个房间添加冷机冷却塔、风机盘管和新风系统。`. Keep the system family and served-room list exactly aligned with this case (["Room1", "Room2", "Room3", "Room4", "Room5"]) unless you intentionally switch to the family workflow for a variant.

## User Prompt And Keywords

- Prompt: `给五个房间添加冷机冷却塔、风机盘管和新风系统。`
- Keywords: Room1-Room5, chiller, cooling tower, DOAS, FCU
...
For Room1 through Room5:

1. Create no-reheat DOAS air terminal.
2. Create matching ThermalZone with the terminal.
3. Create FCU fan, hot-water coil, chilled-water coil, and four-pipe FCU.

Then:

1. Create DOAS air loop with outdoor-air system, supply fan, branches, and
   AirLoopHVAC.
...
Do not use load-profile plant demand, generic PlantLoop, or hand-created
NoAirLoop.
