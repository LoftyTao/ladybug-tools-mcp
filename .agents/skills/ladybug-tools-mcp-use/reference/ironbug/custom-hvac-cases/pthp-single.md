# Case Skill: pthp_single

## Applicable Scenario

Use this case when the request matches the retained prompt: `对 Room1 添加 PTHP，并带电辅助加热。`. Keep the system family and served-room list exactly aligned with this case (["Room1"]) unless you intentionally switch to the family workflow for a variant.

## User Prompt And Keywords

- Prompt: `对 Room1 添加 PTHP，并带电辅助加热。`
- Keywords: Room1, PTHP, packaged terminal heat pump, supplemental electric heat
...
1. Create a matching ThermalZone for Room1.
2. Create one OnOff fan.
3. Create one DX single-speed heating coil.
4. Create one DX single-speed cooling coil.
5. Create one electric supplemental heating coil.
6. Create `detailed_hvac_zone_equipment_pthp` with all
   child targets and the ThermalZone target.
7. Apply, run Energy, read EUI, ERR, and SQL.
...
```jsonc
{
  ...
  "eui": 123.456,
  "err_path": "runs/energy/pthp_single_run/annual_energy_use/run/eplusout.err",
  "sql_path": "runs/energy/pthp_single_run/annual_energy_use/run/eplusout.sql",
  "blocker": null
}
```
...
Do not create water loops, DOAS, AirLoopHVAC, load-profile plant demand, or
