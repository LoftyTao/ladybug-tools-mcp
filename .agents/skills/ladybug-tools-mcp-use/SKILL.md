...
FastMCP Code Mode is still MCP: use the active Agent MCP connection and the host application's MCP tool calls. In Code Mode, the MCP tools exposed to the Agent are `search`, `get_schema`, and `execute`; call domain tools inside `execute` via `await call_tool(...)`.
