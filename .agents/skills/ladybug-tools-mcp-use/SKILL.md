...
- For downstream `target` / `host_target`, prefer the nested typed target dict such as `matches[i].target` or a write tool's returned `target`. The server can auto-unwrap common full responses when exactly one valid target is present; ambiguous responses still fail with a diagnostic error.
