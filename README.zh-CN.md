...
- Python 3.12
- 与下方当前 `[DEV]` 表格匹配的 Ladybug Tools 运行环境
- Git
- uv
...
注意: Ladybug Tools 是一个完整的生态，并且还在积极、持续的更新，因此这个先决条件会随动。下表记录的是截至 2026-05-25，Ladybug Tools MCP 当前 `[DEV]` 状态下期望的运行环境。它参考上游 [Ladybug Tools 兼容性矩阵](https://github.com/ladybug-tools/lbt-grasshopper/wiki/1.4-Compatibility-Matrix) 的形式，去掉 Rhino，增加本项目会通过 `get_ladybug_tools_config` 报告的 THERM 和 Ironbug.Console。
