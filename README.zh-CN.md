...
Ladybug Tools MCP `v1.1.0` 主要收束了两个项目级变化：

- Ironbug 现在通过仓库内的纯 Python API 层表达，不再依赖早期 C# Console 路径完成建模和校验。
- Web View 演示模式的主要可视化入口改为 FastMCP Apps 协议。支持 MCP Apps UI 的宿主可以直接在沙盒 App 面板里渲染 vtk.js 预览。FastMCP 当前 [Apps 文档](https://gofastmcp.com/apps/quickstart)用 Claude Desktop、Goose 等宿主举例说明这种渲染路径；如果宿主还没有声明 Apps UI 扩展，工具会返回本地 `127.0.0.1` fallback URL。
...
- [基本概念](#基本概念)
- [快速开始](#快速开始)
- [FastMCP App 预览模式 Beta](#fastmcp-app-预览模式-beta)
- [初次使用](#初次使用)
- [工作流程示例](#工作流程示例)
- [主要工具](#主要工具)
- [如何贡献](#如何贡献)
...
- Python 3.12
- 与下方当前 `v1.1.0` 表格匹配的 Ladybug Tools 运行环境
- Git
- uv
- 任意智能体应用，例如 [Codex](https://chatgpt.com/codex) , [Claude Code](https://code.claude.com/docs/en/desktop-quickstart) , [Open Code](https://opencode.ai/) 以及 [OpenClaw](https://openclaw.ai/) 等。

如果您并不了解什么是智能体应用，那么我非常乐意推荐您使用 [Codex](https://chatgpt.com/codex)。

注意: Ladybug Tools 是一个完整的生态，并且还在持续更新，因此这个先决条件会随动。下表记录的是截至 2026-06-01，Ladybug Tools MCP `v1.1.0` 源码发布版期望的运行环境。它参考上游 [Ladybug Tools 兼容性矩阵](https://github.com/ladybug-tools/lbt-grasshopper/wiki/1.4-Compatibility-Matrix) 的形式，去掉 Rhino，增加本项目会通过 `get_ladybug_tools_config` 报告的 THERM。Ironbug 建模现在走项目内 Python 层，普通 MCP 工作流不再要求 Ironbug.Console。
...
本项目不会替用户安装这些外部运行时。请按照自己需要的工作流手动选择并安装对应运行时，然后用 `get_ladybug_tools_config` 检查本机 SDK 配置能读取到什么。Config 工具找不到某项运行时时，会返回相关文档链接和安装提示，而不是自动下载或配置。
### 安装指南
...
`requirements.txt` 是固定版本的可复现安装文件。维护者如果希望使用不固定版本的开发依赖，可以改用 `requirements-dev.txt`。
...
## FastMCP App 预览模式 Beta

Web View 演示模式是一个实验性的 FastMCP App 预览模式，适合实时建模过程。它可以让宿主应用在智能体创建或编辑 Honeybee、Dragonfly、Fairyfly 或 VisualizationSet 输出时，通过 vtk.js 渲染当前 Garden 预览。

FastMCP Apps 允许工具返回 UI 资源；当宿主支持 MCP Apps UI 扩展时，宿主会在沙盒 iframe 中渲染这个资源。FastMCP 官方 [Apps quickstart](https://gofastmcp.com/apps/quickstart) 用 Claude Desktop、Goose 等宿主举例说明这种路径，并提供 `fastmcp dev apps` 给开发者做本地浏览器预览。宿主支持程度仍然会变化。如果客户端没有声明 Apps UI 扩展，Ladybug Tools MCP 会返回一个本地 fallback URL，从 `127.0.0.1` 提供同一个 Garden 预览。

### 如何开启

在建模或编辑前，让智能体开启 Web View 演示模式：

```text
为这个 Garden 开启 Web View 演示模式，然后创建或编辑 Honeybee Model。
```
...
开启后，MCP 会创建 Garden 本地 Web View session，并返回给宿主使用的 FastMCP App 元数据。如果宿主不能渲染 App iframe，结果里也会包含本地 `viewer.url`，通常是：

```text
http://127.0.0.1:3127
```

MCP 服务不会强行打开系统默认浏览器。支持 MCP Apps 的宿主应该渲染返回的 `ui://web_view/ladybug-tools/vtkjs-preview.html` 资源。不支持该扩展的宿主，包括当前会报告 `client_supports_ui_extension=false` 的 Codex 运行，可以在需要可见预览时打开返回的 fallback URL。

### 如何关闭

让智能体停止 Web View 演示模式，或调用：

```text
stop_web_view_mode(garden_root)
```

这会停止后续自动预览，并关闭对应的本地 fallback viewer。`tmp/web_view/` 下的预览历史会保留。

### 与普通模式的区别

普通模式下，建模工具只写入 Garden 文件，并返回紧凑的目标引用、摘要和持久化回执。它不会启动浏览器服务，也不会在每次编辑后自动导出预览文件。

Web View 演示模式会多出一条本地视觉反馈链路。创建 Room、编辑 Room、添加 Aperture、添加 Door、移动对象、求解相邻关系等显著 Honeybee、Dragonfly、Fairyfly 和 VisualizationSet 操作之后，Code Mode 会自动导出 session 管理的 `.vtkjs` 预览到：

```text
<garden>/tmp/web_view/previews/
```

App 会轮询 Garden session 状态，并自动加载最新的 `.vtkjs` 包，不需要手动刷新。这些自动预览只是本地 session 状态，不是用户显式要求的正式 Garden artifact。如果需要可复用、可交付的正式 vtk.js 资产，仍然应该让智能体使用 `visualization_set_to_vtkjs` 导出 VisualizationSet。

fallback viewer 故意使用明确的本地端口。如果端口已被占用，启动会直接失败并说明原因，而不是悄悄换端口或让浏览器继续停在旧 Garden 上。

## 初次使用

在智能体应用中配置好 MCP 服务后，新开一个线程并让它使用 Ladybug Tools MCP 即可。在 Codex 中，最直接的方式是按上面的 TOML 示例把 server 写入 `~/.codex/config.toml`，重启 Codex，然后直接描述 Garden 或建模任务。

如果宿主应用支持技能，可以在 Ladybug Tools MCP 启用后，通过 `/` 触发 `ladybug-tools-mcp-use` 技能，再输入 `你好，Ladybug Tools！` 来激活我们提供的三种主要使用意图的引导流程；在引导结束之后，您就可以开始进行对应的任务构建。
...
### 用 Ironbug 创建自定义空调系统
  ...
  - 创建 Ironbug DetailedHVAC（细颗粒度空调系统）对象，包括 Coil、Fan、Pump、Boiler、Chiller、Terminal、Plant Loop、Air Loop、Setpoint Manager 和 Output Request。
  - 组装有来源依据的自定义 HVAC 系统，例如 PTAC、PTHP、FCU、DOAS、VAV、VRF、锅炉再热、冷机机房和冷却水环路。
  - 将 Ironbug ThermalZone 关联到 Honeybee 或 Dragonfly 房间，再把 DetailedHVAC 模型应用到建筑模型，之后继续走标准 Energy 模拟流程。
  - 当 HVAC Template 太粗，您需要控制环路拓扑、子部件和面向 OpenStudio / EnergyPlus 的设备意图时，使用 Ironbug 工作流。
...
- 创建 Fan、Pump、Coil、Boiler、Chiller、Heat Exchanger、Setpoint Manager、Availability Manager、Terminal、Zone Equipment 和 Plant Equipment 等 DetailedHVAC 部件
- 用有来源依据的供给侧和需求侧部件创建 Hot Water Loop、Chilled Water Loop 和 Condenser Water Loop
- 创建 AirLoop、PlantLoop、ThermalZone、OutdoorAirSystem、Existing Object、Node Probe、EMS 和 Output Variable 记录
- 搜索、校验和组装保存在 Garden 中的 Ironbug Model
- 在 Room 的 Energy 属性和 Setpoint 就绪后，为 Energy 模拟准备自定义 HVAC 系统
...
- [ ] 扩展 Ironbug Energy 保留验收案例，覆盖更多自定义 HVAC 拓扑
