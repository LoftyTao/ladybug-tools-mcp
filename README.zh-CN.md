...
- [基本概念](#基本概念)
- [快速开始](#快速开始)
- [Web View 演示模式 Beta](#web-view-演示模式-beta)
- [初次使用](#初次使用)
- [工作流程示例](#工作流程示例)
- [主要工具](#主要工具)
- [如何贡献](#如何贡献)
...
- Python 3.12
- 与下方当前 `v1.0.0` 表格匹配的 Ladybug Tools 运行环境
- Git
- uv
- 如果要体验实验性的 Web View 演示模式 Beta，还需要 Node.js 20+
...
注意: Ladybug Tools 是一个完整的生态，并且还在持续更新，因此这个先决条件会随动。下表记录的是截至 2026-05-27，Ladybug Tools MCP `v1.0.0` 源码发布版期望的运行环境。它参考上游 [Ladybug Tools 兼容性矩阵](https://github.com/ladybug-tools/lbt-grasshopper/wiki/1.4-Compatibility-Matrix) 的形式，去掉 Rhino，增加本项目会通过 `get_ladybug_tools_config` 报告的 THERM 和 Ironbug.Console。
...
如果要体验实验性的 Web View 演示模式 Beta，还需要安装它的前端依赖：
...
Web View 演示模式是一个实验性的本地预览模式，适合实时建模过程。它可以让已经打开的浏览器随着智能体对 Garden 中 Honeybee 对象的创建和编辑自动更新。
...
开启后，MCP 会创建 Garden 本地 Web View session，启动本地 React + vtk.js server，启动 Garden watcher，并返回本地 `viewer.url`，通常是：

```text
http://127.0.0.1:3127
```

MCP 服务不会强行打开系统默认浏览器。Codex 这样的宿主应用如果具备侧边栏浏览器能力，应该由宿主把返回的 URL 打开到侧边栏中。
...
这会停止后续自动预览，并关闭对应的本地 viewer server / watcher。`tmp/web_view/` 下的预览历史会保留。

### 与普通模式的区别

普通模式下，建模工具只写入 Garden 文件，并返回紧凑的目标引用、摘要和持久化回执。它不会启动浏览器服务，也不会在每次编辑后自动导出预览文件。

Web View 演示模式会多出一条本地视觉反馈链路。创建 Room、编辑 Room、添加 Aperture、添加 Door、移动对象、求解相邻关系等显著 Honeybee 写操作之后，Code Mode 会自动导出 session 管理的 `.vtkjs` 预览到：

```text
<garden>/tmp/web_view/previews/
```

viewer 会轮询 `config.json`，并自动加载最新的 `.vtkjs` 包，不需要手动刷新浏览器。这些自动预览只是本地 session 状态，不是用户显式要求的正式 Garden artifact。如果需要可复用、可交付的正式 vtk.js 资产，仍然应该让智能体使用 `visualization_set_to_vtkjs` 导出 VisualizationSet。

这个 Beta 版本故意使用明确的本地端口。如果端口已被占用，启动会直接失败并说明原因，而不是悄悄换端口或让浏览器继续停在旧 Garden 上。
