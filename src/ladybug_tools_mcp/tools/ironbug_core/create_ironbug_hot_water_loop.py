"""Create an Ironbug hot-water loop MCP tool."""
...
def register(mcp: FastMCP) -> None:
    'Register the detailed_hvac_plant_loop_hot_water tool.'

    @mcp.tool(
        name="plant_loop_hot_water",
        description=(
            "Create a reviewed hot-water Ironbug loop from pump/source/load "
            "component targets. This is the public heating-loop authoring path; "
            "it does not expose generic PlantLoop or explicit OperationScheme "
            "inputs. If exactly one pump is present, it is placed as a direct "
            "loop supply component before the supply branch group. If no pump "
            "is present in supply_branch_component_targets, the service adds a "
            "source-backed default IB_PumpConstantSpeed for EnergyPlus-ready "
            "topology. Apply DetailedHVAC to Honeybee or Dragonfly, then run "
            "the standard Ladybug Tools MCP Energy workflow."
        ),
        tags={"ironbug", "plant-loop", "hot-water", "author"},
        timeout=20,
    )
    def create_ironbug_hot_water_loop(
        ...
        identifier: Annotated[
            str,
            Field(description="Stable identifier for the hot-water loop."),
        ],
        loop_name: Annotated[
            str | None,
        ...
        supply_branch_component_targets: Annotated[
            ...
            Field(
                description=(
                    "Ordered supply-side pump and heating-source component targets "
                    ...
                    "and keeps the remaining source equipment in branch groups. "
                    "A flat list is one serial equipment branch; a nested list "
                    "such as [[source_target_1], [source_target_2, "
        ...
        demand_branch_component_targets: Annotated[
            ...
            Field(
                description=(
                    "Ordered demand-side load or hydronic-coil component targets "
                    ...
                    "loads, exchangers, tanks, or boilers should be parallel, "
        ...
        setpoint_c: Annotated[
            float,
            Field(description="Hot-water loop temperature setpoint in Celsius."),
        ] = 60.0,
        ...
        overwrite: Annotated[
            bool,
            Field(description="Replace an existing hot-water loop with the same identifier."),
        ] = False,
    ) -> dict[str, Any]:
        """Create a hot-water Ironbug loop."""
